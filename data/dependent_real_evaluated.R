...
#############################################
#############################################
# Set the wprking directory
setwd("/home/hp/Documents/GitHub/16S_postprocessing/Nur_Code/the comparison")

WD = "~/Documents/GitHub/16S_postprocessing/Nur_Code/the comparison"
library(phyloseq)
#############################################
...
###########################################
###########################################
edgeRRobust <- function(physeq, design = as.formula("~ 0 + group + time + group:time"), prior.df = 10, 
                        normFacts = c("TMM", "RLE", "ratio", "CSS", "UQ", "none", "SAM", "TSS"), returnDispEsts = FALSE)
{
  ...
  ##for (i in 1:length(physeq@sam_data$time)){ physeq@sam_data$time[i] <- paste0(physeq@sam_data$time[i], "h") }  
  target <- data.frame(time = physeq@sam_data$time,group=physeq@sam_data$group)   
  rownames(target) = rownames(physeq@sam_data)
  ...
  glmRes <- glmQLFTest(glmFit,coef=4)
...
###################################
## Deseq2
negBinTestDESeq2 <- function(physeq, design = as.formula("~  group + time + group:time "), IndepFilter = NULL,
                             normFacts = c("TMM", "RLE", "ratio", "CSS", "UQ", "none", "SAM", "TSS"), returnDispEsts = FALSE)
{
  ...
  physeq@sam_data$group = factor(physeq@sam_data$group)
  ...
  dds = nbinomLRT(dds, reduced = ~ group + time )
  res <- results(dds)
  out = cbind("rawP" = res$pvalue, "adjP" = res$padj)
  ...
  #   suppressWarnings(
  #     ddsDisp <- try(
  #       estimateDispersions(dds, fitType = 'local', quiet = TRUE)
  #     ,silent = TRUE))
  #     #dds <- estimateDispersionsGeneEst(dds)
  #     #dispersions(dds) <- mcols(dds)$dispGeneEst
  #     #ddsDisp = dispersions(dds)
  #     
...
limmaVoomRobust <- function (physeq, design = as.formula("~ 0 + group + time + group:time"), 
                             normFacts = c("TMM", "RLE", "ratio", "CSS", "UQ", "none", "SAM", "TSS"))
{
  ...
  pval <- topTable(fitRes, coef = 4, n = nrow(counts), sort.by = "none")$P.Value
  padj <- topTable(fitRes, coef = 4, n = nrow(counts), sort.by = "none")$adj.P.Val
  out = cbind("rawP" = pval, "adjP" = padj)
  rownames(out)= taxa_names(physeq)
  out
}# END: limmaVoomRobust
...
TimeSeries_metagenomeSeqZIG <- function (physeq, design = as.formula("~ 0+group+time+group:time"), 
                                         normFacts = c("TMM", "RLE", "ratio", "CSS", "UQ", "none", "SAM", "TSS"))
{
  # require(metagenomeSeq)
  ### force orientation OTUs x samples
...
TimeSeries_aldexTest = function(physeq, mc.samples = 128){
  # require(ALDEx2)
  if (!taxa_are_rows(physeq)){
    physeq=t(physeq)
  }
  #data=data.frame(otu_table(physeq)@.Data)
  data <- as(otu_table(physeq), "matrix")
  target <- data.frame(time = physeq@sam_data$time,group=physeq@sam_data$group)   
  rownames(target) = rownames(physeq@sam_data)
  mm <- model.matrix(~ group * time , data = target)
  
  # you need to round the normalized abundance before enter them to aldex.clr!!!
  # get more info from this question => https://forum.qiime2.org/t/aldex2-error-when-processing-picrust2-output/14104
  x <- aldex.clr(round(data, digits = 0), mm, mc.samples=mc.samples, denom="all", verbose=F)
  glm.test <- aldex.glm(x, mm)
  #res = aldex(data, conditions = sample_data(physeq)$group,mc.samples=mc.samples, test ="t")
  out = cbind(glm.test$'model.groupTreatment:time Pr(>|t|)',  p.adjust(glm.test$'model.groupTreatment:time Pr(>|t|)', method = "BH"))
  colnames(out) = c("rawP","adjP")
  rownames(out) = rownames(glm.test) #taxa_names(physeq)
  out = data.matrix(out)
  return(out)
}
###############################
TimeSeries_ancombcTest2 = function(physeq){
  ...
  # create adjp fdr
  outPvalue = do.call(rbind, Map(data.frame, rawP= out$res[,17], adjP= out$res[,21]))
  rownames(outPvalue) = out$res$taxon
  outPvalue = data.matrix(outPvalue)
  outPvalue
}
###############################
ancomTimeseries <- function(physeq){
  library(nlme)
  library(tidyverse)
  ...
  feature_table = otu_data; sample_var = "Sample_ID"; group_var = "group"
  ...
  adj_formula = "time"; rand_formula = "~ time | ID"
...
aldexTest = function(physeq, mc.samples = 128){
  ...
  #res = aldex(data, conditions = sample_data(physeq)$group,mc.samples=mc.samples, test ="t")
  #out = cbind(glm.test$'model.groupTreatment:time Pr(>|t|)',  p.adjust(glm.test$'model.groupTreatment:time Pr(>|t|)', method = "BH"))
  out = cbind(glm.test$'groupgrp2:timeD6:pval',  p.adjust(glm.test$'groupgrp2:timeD6:pval', method = "BH"))
  colnames(out) = c("rawP","adjP")
  rownames(out) = rownames(glm.test) #taxa_names(physeq)
  out = data.matrix(out)
  return(out)
...
twoWayANOVA <- function(physeq, whichOTUs = NULL,
                        ...
                        alt = "two.sided", adjMethod = "BH")
{
  ...
  res <- numeric(length = nrow(counts))
  for (x in 1:nrow(counts)){
    Pval=try(
      summary(aov(counts[x,] ~ physeq@sam_data$time * physeq@sam_data$group))[[1]][["Pr(>F)"]][3]
      , silent=TRUE)
  ...
  FinalResult = matrix(nrow = nrow(physeq@otu_table), ncol = 2)
  FinalResult[,1] <- pVals
  FinalResult[,2] <- adjPVals
  FinalResult = as.data.frame(FinalResult)
  rownames(FinalResult) <- rownames(physeq@otu_table)
  colnames(FinalResult)[1] <- "rawP"
  colnames(FinalResult)[2] <- "adjP"
  FinalResult
...
# load(file = "list_ANTICIPATE_AB.RData")
# simListH1gs = list_ANTICIPATE_AB[2:117]

# for (sub in 1:length(simListH1gs) ){
#   simListH1gs[[sub]] = prune_taxa(sample(taxa_names(simListH1gs[[sub]]),100), simListH1gs[[sub]])
# }


setwd("~/Documents/GitHub/16S_postprocessing/Nur_Code/the comparison")
#load(file = './list_ANTICIPATE_AB.Edit100taxa.RData')


load(file = "list_ANTICIPATE_AB.RData")

simListH1gs = list_ANTICIPATE_AB

##load(file = "./FinalResults/simListH1gs100taxaAllSamples.RData")
...
twoWayANOVA_twoWayANOVA.list <- list()

library(phyloseq)
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  tryCatch({
    # consider that the whole data is verification
    tax_table(simListH1gs[[nu]]) <- NULL
    simListH1gs_verif = simListH1gs[[nu]]

    # divide the data into 5 parts and each part is evaluated
    GeneralmatrixEval_twoWayANOVA_twoWayANOVA = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 3)
    GeneralmatrixVerif_twoWayANOVA_twoWayANOVA = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 1)

    simListH1gs_verif = normEdgeR(physeq = simListH1gs_verif, method = "TMM")
    DAListH1GSverif = twoWayANOVA(simListH1gs_verif, normFacts = "TMM")
    matrixVerif = matrix(nrow = nrow(simListH1gs_verif@otu_table), ncol = 1)
    rownames(matrixVerif) = rownames(simListH1gs_verif@otu_table)
    colnames(matrixVerif) = "twoWayANOVA_twoWayANOVA"
    DAListH1GSverif[,2] <- as.logical(DAListH1GSverif[,2] < 0.05)
    for ( x in 1:nrow(DAListH1GSverif)){
      matrixVerif[ rownames(DAListH1GSverif)[x] ,] = as.logical(DAListH1GSverif[,2][x])
    }
    matrixVerif = data.frame(matrixVerif)
    GeneralmatrixVerif_twoWayANOVA_twoWayANOVA[,1] = matrixVerif[,1] # 1 in number of test
    rownames(GeneralmatrixVerif_twoWayANOVA_twoWayANOVA) = rownames(matrixVerif)
    GeneralmatrixVerif_twoWayANOVA_twoWayANOVA = data.frame(GeneralmatrixVerif_twoWayANOVA_twoWayANOVA)

    for (iter in 1:3){
      tax_table(simListH1gs[[nu]]) <- NULL
      simListH1gs_eval = simListH1gs[[nu]]
      # simListH1gs_verif = simListH1gs[[nu]]
      ...
      DAListH1GSeval =  tryCatch(
        twoWayANOVA(simListH1gs_eval, normFacts = "TMM"),
        error = function(e) e = matrix(nrow =  nrow(simListH1gs_eval@otu_table), ncol = 2)
      )
      # DAListH1GSeval =  twoWayANOVA(simListH1gs_eval)
      # DAListH1GSverif = twoWayANOVA(simListH1gs_verif)
      matrixEval = matrix(nrow = nrow(simListH1gs_eval@otu_table), ncol = 1  )
      rownames(matrixEval) = rownames(simListH1gs_eval@otu_table)
      colnames(matrixEval) = "twoWayANOVA_twoWayANOVA"
      #for (d in 1:length(names(DAListH1GSeval))){
      DAListH1GSeval[,2] <- as.logical(DAListH1GSeval[,2] < 0.05)
      for ( x in 1:nrow(DAListH1GSeval)){
        matrixEval[ rownames(DAListH1GSeval)[x] ,] = as.logical(DAListH1GSeval[,2][x])
      }


      matrixEval = data.frame(matrixEval)
      # matrixVerif = data.frame(matrixVerif)

      GeneralmatrixEval_twoWayANOVA_twoWayANOVA[,iter] = matrixEval[,1] # 1 in number of test
      # GeneralmatrixVerif_twoWayANOVA_twoWayANOVA[,iter] = matrixVerif[,1] # 1 in number of test

      rownames(GeneralmatrixEval_twoWayANOVA_twoWayANOVA) = rownames(matrixEval)
      # rownames(GeneralmatrixVerif_twoWayANOVA_twoWayANOVA) = rownames(matrixVerif)
      GeneralmatrixEval_twoWayANOVA_twoWayANOVA = data.frame(GeneralmatrixEval_twoWayANOVA_twoWayANOVA)
      # GeneralmatrixVerif_twoWayANOVA_twoWayANOVA = data.frame(GeneralmatrixVerif_twoWayANOVA_twoWayANOVA)
    }
    GeneralmatrixVerif_twoWayANOVA_twoWayANOVA[is.na(GeneralmatrixVerif_twoWayANOVA_twoWayANOVA)] = FALSE # replace NA with FALSE
    GeneralmatrixEval_twoWayANOVA_twoWayANOVA[is.na(GeneralmatrixEval_twoWayANOVA_twoWayANOVA)] = FALSE # replace NA with FALSE

    TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA = matrix(nrow = nrow(GeneralmatrixEval_twoWayANOVA_twoWayANOVA),ncol = 4)

    rownames(TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA) = rownames(GeneralmatrixEval_twoWayANOVA_twoWayANOVA)
    # colnames(TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA) = c("TruthVerif","Confusion1","Confusion2","Confusion3","Confusion4","Confusion5","GeneralConfusion")
    colnames(TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA) = c("TruthVerif","Confusion1","Confusion2","Confusion3")


    TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[,1] <- GeneralmatrixVerif_twoWayANOVA_twoWayANOVA[,1]

    for (col in 1:ncol(GeneralmatrixEval_twoWayANOVA_twoWayANOVA)){
      for (W in 1:nrow(TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA)){
        if (TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,1] == "Gray" && as.logical(GeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col]) == TRUE){
          TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col+1]  = "GrayTrue"
        }else if (TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,1] == "Gray" && as.logical(GeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col]) == FALSE){
          TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col+1]  = "GrayFalse"
        }else if ( as.logical(TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,1]) == TRUE && as.logical(GeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col]) == TRUE){
          TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col+1] = "TP"
        } else if (as.logical(TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,1]) == FALSE && as.logical(GeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col]) == FALSE){
          TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col+1]  = "TN"
        } else if (as.logical(TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,1]) == TRUE && as.logical(GeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col]) == FALSE){
          TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col+1]  = "FN"
        } else if (as.logical(TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,1]) == FALSE && as.logical(GeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col]) == TRUE){
          TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA[W,col+1]  = "FP"
        }
      }
    }


    twoWayANOVA_twoWayANOVA.list[[nu]] <- TruthGeneralmatrixEval_twoWayANOVA_twoWayANOVA
  },
  error = function(e) {
...
result.twoWayANOVA_twoWayANOVA.list <- do.call(rbind, twoWayANOVA_twoWayANOVA.list)

save(result.twoWayANOVA_twoWayANOVA.list, file= "./FinalResults/twoWayANOVA_twoWayANOVA.listTest6.RData")
...
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  tryCatch({
    ...
    # divide the data into 5 parts and each part is evaluated
    GeneralmatrixEval_edgeR_edgeR = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 3)
    GeneralmatrixVerif_edgeR_edgeR = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 1)

    simListH1gs_verif = normEdgeR(physeq = simListH1gs_verif, method = "TMM")
    DAListH1GSverif = edgeRRobust(simListH1gs_verif, normFacts = "TMM")
    matrixVerif = matrix(nrow = nrow(simListH1gs_verif@otu_table), ncol = 1)
    rownames(matrixVerif) = rownames(simListH1gs_verif@otu_table)
    colnames(matrixVerif) = "edgeR_edgeR"
    DAListH1GSverif[,2] <- as.logical(DAListH1GSverif[,2] < 0.05)
    for ( x in 1:nrow(DAListH1GSverif)){
      matrixVerif[ rownames(DAListH1GSverif)[x] ,] = as.logical(DAListH1GSverif[,2][x])
    }
    matrixVerif = data.frame(matrixVerif)
    GeneralmatrixVerif_edgeR_edgeR[,1] = matrixVerif[,1] # 1 in number of test
    rownames(GeneralmatrixVerif_edgeR_edgeR) = rownames(matrixVerif)
    GeneralmatrixVerif_edgeR_edgeR = data.frame(GeneralmatrixVerif_edgeR_edgeR)
    ...
    for (iter in 1:3){
      ...
      # DAListH1GSeval =  edgeR(simListH1gs_eval)
      # DAListH1GSverif = edgeR(simListH1gs_verif)
      matrixEval = matrix(nrow = nrow(simListH1gs_eval@otu_table), ncol = 1  )
      rownames(matrixEval) = rownames(simListH1gs_eval@otu_table)
      colnames(matrixEval) = "edgeR_edgeR"
      #for (d in 1:length(names(DAListH1GSeval))){
      DAListH1GSeval[,2] <- as.logical(DAListH1GSeval[,2] < 0.05)
      for ( x in 1:nrow(DAListH1GSeval)){
        matrixEval[ rownames(DAListH1GSeval)[x] ,] = as.logical(DAListH1GSeval[,2][x])
      ...
      GeneralmatrixEval_edgeR_edgeR[,iter] = matrixEval[,1] # 1 in number of test
      # GeneralmatrixVerif_edgeR_edgeR[,iter] = matrixVerif[,1] # 1 in number of test

      rownames(GeneralmatrixEval_edgeR_edgeR) = rownames(matrixEval)
      # rownames(GeneralmatrixVerif_edgeR_edgeR) = rownames(matrixVerif)
      GeneralmatrixEval_edgeR_edgeR = data.frame(GeneralmatrixEval_edgeR_edgeR)
      # GeneralmatrixVerif_edgeR_edgeR = data.frame(GeneralmatrixVerif_edgeR_edgeR)
    }
    GeneralmatrixVerif_edgeR_edgeR[is.na(GeneralmatrixVerif_edgeR_edgeR)] = FALSE # replace NA with FALSE
    GeneralmatrixEval_edgeR_edgeR[is.na(GeneralmatrixEval_edgeR_edgeR)] = FALSE # replace NA with FALSE

    TruthGeneralmatrixEval_edgeR_edgeR = matrix(nrow = nrow(GeneralmatrixEval_edgeR_edgeR),ncol = 4)

    rownames(TruthGeneralmatrixEval_edgeR_edgeR) = rownames(GeneralmatrixEval_edgeR_edgeR)
    # colnames(TruthGeneralmatrixEval_edgeR_edgeR) = c("TruthVerif","Confusion1","Confusion2","Confusion3","Confusion4","Confusion5","GeneralConfusion")
    colnames(TruthGeneralmatrixEval_edgeR_edgeR) = c("TruthVerif","Confusion1","Confusion2","Confusion3")


    TruthGeneralmatrixEval_edgeR_edgeR[,1] <- GeneralmatrixVerif_edgeR_edgeR[,1]

    for (col in 1:ncol(GeneralmatrixEval_edgeR_edgeR)){
      for (W in 1:nrow(TruthGeneralmatrixEval_edgeR_edgeR)){
        if (TruthGeneralmatrixEval_edgeR_edgeR[W,1] == "Gray" && as.logical(GeneralmatrixEval_edgeR_edgeR[W,col]) == TRUE){
          TruthGeneralmatrixEval_edgeR_edgeR[W,col+1]  = "GrayTrue"
        }else if (TruthGeneralmatrixEval_edgeR_edgeR[W,1] == "Gray" && as.logical(GeneralmatrixEval_edgeR_edgeR[W,col]) == FALSE){
          TruthGeneralmatrixEval_edgeR_edgeR[W,col+1]  = "GrayFalse"
        }else if ( as.logical(TruthGeneralmatrixEval_edgeR_edgeR[W,1]) == TRUE && as.logical(GeneralmatrixEval_edgeR_edgeR[W,col]) == TRUE){
          TruthGeneralmatrixEval_edgeR_edgeR[W,col+1] = "TP"
        } else if (as.logical(TruthGeneralmatrixEval_edgeR_edgeR[W,1]) == FALSE && as.logical(GeneralmatrixEval_edgeR_edgeR[W,col]) == FALSE){
          TruthGeneralmatrixEval_edgeR_edgeR[W,col+1]  = "TN"
        } else if (as.logical(TruthGeneralmatrixEval_edgeR_edgeR[W,1]) == TRUE && as.logical(GeneralmatrixEval_edgeR_edgeR[W,col]) == FALSE){
          TruthGeneralmatrixEval_edgeR_edgeR[W,col+1]  = "FN"
        } else if (as.logical(TruthGeneralmatrixEval_edgeR_edgeR[W,1]) == FALSE && as.logical(GeneralmatrixEval_edgeR_edgeR[W,col]) == TRUE){
          TruthGeneralmatrixEval_edgeR_edgeR[W,col+1]  = "FP"
        }
      }
    }


    edgeR_edgeR.list[[nu]] <- TruthGeneralmatrixEval_edgeR_edgeR
  },
  error = function(e) {
    # if an error occurs, print a message and skip to the next iteration
    message("Error: ", e$message)
...
result.edgeR_edgeR.list <- do.call(rbind, edgeR_edgeR.list)

save(result.edgeR_edgeR.list, file= "./FinalResults/edgeR_edgeR.listTest6.RData")
...
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  tryCatch({
    ...
    # divide the data into 5 parts and each part is evaluated
    GeneralmatrixEval_deseq2_deseq2 = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 3)
    GeneralmatrixVerif_deseq2_deseq2 = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 1)

    simListH1gs_verif = normEdgeR(physeq = simListH1gs_verif, method = "TMM")
    DAListH1GSverif = negBinTestDESeq2(simListH1gs_verif, normFacts = "TMM")
    matrixVerif = matrix(nrow = nrow(simListH1gs_verif@otu_table), ncol = 1)
    rownames(matrixVerif) = rownames(simListH1gs_verif@otu_table)
    colnames(matrixVerif) = "deseq2_deseq2"
    DAListH1GSverif[,2] <- as.logical(DAListH1GSverif[,2] < 0.05)
    for ( x in 1:nrow(DAListH1GSverif)){
      matrixVerif[ rownames(DAListH1GSverif)[x] ,] = as.logical(DAListH1GSverif[,2][x])
    }
    matrixVerif = data.frame(matrixVerif)
    GeneralmatrixVerif_deseq2_deseq2[,1] = matrixVerif[,1] # 1 in number of test
    rownames(GeneralmatrixVerif_deseq2_deseq2) = rownames(matrixVerif)
    GeneralmatrixVerif_deseq2_deseq2 = data.frame(GeneralmatrixVerif_deseq2_deseq2)
    ...
    for (iter in 1:3){
      ...
      # DAListH1GSeval =  deseq2(simListH1gs_eval)
      # DAListH1GSverif = deseq2(simListH1gs_verif)
      matrixEval = matrix(nrow = nrow(simListH1gs_eval@otu_table), ncol = 1  )
      rownames(matrixEval) = rownames(simListH1gs_eval@otu_table)
      colnames(matrixEval) = "deseq2_deseq2"
      #for (d in 1:length(names(DAListH1GSeval))){
      DAListH1GSeval[,2] <- as.logical(DAListH1GSeval[,2] < 0.05)
      for ( x in 1:nrow(DAListH1GSeval)){
        matrixEval[ rownames(DAListH1GSeval)[x] ,] = as.logical(DAListH1GSeval[,2][x])
      ...
      GeneralmatrixEval_deseq2_deseq2[,iter] = matrixEval[,1] # 1 in number of test
      # GeneralmatrixVerif_deseq2_deseq2[,iter] = matrixVerif[,1] # 1 in number of test

      rownames(GeneralmatrixEval_deseq2_deseq2) = rownames(matrixEval)
      # rownames(GeneralmatrixVerif_deseq2_deseq2) = rownames(matrixVerif)
      GeneralmatrixEval_deseq2_deseq2 = data.frame(GeneralmatrixEval_deseq2_deseq2)
      # GeneralmatrixVerif_deseq2_deseq2 = data.frame(GeneralmatrixVerif_deseq2_deseq2)
    }
    GeneralmatrixVerif_deseq2_deseq2[is.na(GeneralmatrixVerif_deseq2_deseq2)] = FALSE # replace NA with FALSE
    GeneralmatrixEval_deseq2_deseq2[is.na(GeneralmatrixEval_deseq2_deseq2)] = FALSE # replace NA with FALSE

    TruthGeneralmatrixEval_deseq2_deseq2 = matrix(nrow = nrow(GeneralmatrixEval_deseq2_deseq2),ncol = 4)

    rownames(TruthGeneralmatrixEval_deseq2_deseq2) = rownames(GeneralmatrixEval_deseq2_deseq2)
    # colnames(TruthGeneralmatrixEval_deseq2_deseq2) = c("TruthVerif","Confusion1","Confusion2","Confusion3","Confusion4","Confusion5","GeneralConfusion")
    colnames(TruthGeneralmatrixEval_deseq2_deseq2) = c("TruthVerif","Confusion1","Confusion2","Confusion3")


    TruthGeneralmatrixEval_deseq2_deseq2[,1] <- GeneralmatrixVerif_deseq2_deseq2[,1]

    for (col in 1:ncol(GeneralmatrixEval_deseq2_deseq2)){
      for (W in 1:nrow(TruthGeneralmatrixEval_deseq2_deseq2)){
        if (TruthGeneralmatrixEval_deseq2_deseq2[W,1] == "Gray" && as.logical(GeneralmatrixEval_deseq2_deseq2[W,col]) == TRUE){
          TruthGeneralmatrixEval_deseq2_deseq2[W,col+1]  = "GrayTrue"
        }else if (TruthGeneralmatrixEval_deseq2_deseq2[W,1] == "Gray" && as.logical(GeneralmatrixEval_deseq2_deseq2[W,col]) == FALSE){
          TruthGeneralmatrixEval_deseq2_deseq2[W,col+1]  = "GrayFalse"
        }else if ( as.logical(TruthGeneralmatrixEval_deseq2_deseq2[W,1]) == TRUE && as.logical(GeneralmatrixEval_deseq2_deseq2[W,col]) == TRUE){
          TruthGeneralmatrixEval_deseq2_deseq2[W,col+1] = "TP"
        } else if (as.logical(TruthGeneralmatrixEval_deseq2_deseq2[W,1]) == FALSE && as.logical(GeneralmatrixEval_deseq2_deseq2[W,col]) == FALSE){
          TruthGeneralmatrixEval_deseq2_deseq2[W,col+1]  = "TN"
        } else if (as.logical(TruthGeneralmatrixEval_deseq2_deseq2[W,1]) == TRUE && as.logical(GeneralmatrixEval_deseq2_deseq2[W,col]) == FALSE){
          TruthGeneralmatrixEval_deseq2_deseq2[W,col+1]  = "FN"
        } else if (as.logical(TruthGeneralmatrixEval_deseq2_deseq2[W,1]) == FALSE && as.logical(GeneralmatrixEval_deseq2_deseq2[W,col]) == TRUE){
          TruthGeneralmatrixEval_deseq2_deseq2[W,col+1]  = "FP"
        }
      }
    }


    deseq2_deseq2.list[[nu]] <- TruthGeneralmatrixEval_deseq2_deseq2
  },
  error = function(e) {
    # if an error occurs, print a message and skip to the next iteration
    message("Error: ", e$message)
...
result.deseq2_deseq2.list <- do.call(rbind, deseq2_deseq2.list)

save(result.deseq2_deseq2.list, file= "./FinalResults/deseq2_deseq2.listTest6.RData")
...
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  tryCatch({
    ...
    # divide the data into 5 parts and each part is evaluated
    GeneralmatrixEval_voomTest_voomTest = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 3)
    GeneralmatrixVerif_voomTest_voomTest = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 1)

    simListH1gs_verif = normEdgeR(physeq = simListH1gs_verif, method = "TMM")
    DAListH1GSverif = limmaVoomRobust(simListH1gs_verif, normFacts = "TMM")
    matrixVerif = matrix(nrow = nrow(simListH1gs_verif@otu_table), ncol = 1)
    rownames(matrixVerif) = rownames(simListH1gs_verif@otu_table)
    colnames(matrixVerif) = "voomTest_voomTest"
    DAListH1GSverif[,2] <- as.logical(DAListH1GSverif[,2] < 0.05)
    for ( x in 1:nrow(DAListH1GSverif)){
      matrixVerif[ rownames(DAListH1GSverif)[x] ,] = as.logical(DAListH1GSverif[,2][x])
    }
    matrixVerif = data.frame(matrixVerif)
    GeneralmatrixVerif_voomTest_voomTest[,1] = matrixVerif[,1] # 1 in number of test
    rownames(GeneralmatrixVerif_voomTest_voomTest) = rownames(matrixVerif)
    GeneralmatrixVerif_voomTest_voomTest = data.frame(GeneralmatrixVerif_voomTest_voomTest)
    ...
    for (iter in 1:3){
      ...
      # DAListH1GSeval =  voomTest(simListH1gs_eval)
      # DAListH1GSverif = voomTest(simListH1gs_verif)
      matrixEval = matrix(nrow = nrow(simListH1gs_eval@otu_table), ncol = 1  )
      rownames(matrixEval) = rownames(simListH1gs_eval@otu_table)
      colnames(matrixEval) = "voomTest_voomTest"
      #for (d in 1:length(names(DAListH1GSeval))){
      DAListH1GSeval[,2] <- as.logical(DAListH1GSeval[,2] < 0.05)
      for ( x in 1:nrow(DAListH1GSeval)){
        matrixEval[ rownames(DAListH1GSeval)[x] ,] = as.logical(DAListH1GSeval[,2][x])
      ...
      GeneralmatrixEval_voomTest_voomTest[,iter] = matrixEval[,1] # 1 in number of test
      # GeneralmatrixVerif_voomTest_voomTest[,iter] = matrixVerif[,1] # 1 in number of test

      rownames(GeneralmatrixEval_voomTest_voomTest) = rownames(matrixEval)
      # rownames(GeneralmatrixVerif_voomTest_voomTest) = rownames(matrixVerif)
      GeneralmatrixEval_voomTest_voomTest = data.frame(GeneralmatrixEval_voomTest_voomTest)
      # GeneralmatrixVerif_voomTest_voomTest = data.frame(GeneralmatrixVerif_voomTest_voomTest)
    }
    GeneralmatrixVerif_voomTest_voomTest[is.na(GeneralmatrixVerif_voomTest_voomTest)] = FALSE # replace NA with FALSE
    GeneralmatrixEval_voomTest_voomTest[is.na(GeneralmatrixEval_voomTest_voomTest)] = FALSE # replace NA with FALSE

    TruthGeneralmatrixEval_voomTest_voomTest = matrix(nrow = nrow(GeneralmatrixEval_voomTest_voomTest),ncol = 4)

    rownames(TruthGeneralmatrixEval_voomTest_voomTest) = rownames(GeneralmatrixEval_voomTest_voomTest)
    # colnames(TruthGeneralmatrixEval_voomTest_voomTest) = c("TruthVerif","Confusion1","Confusion2","Confusion3","Confusion4","Confusion5","GeneralConfusion")
    colnames(TruthGeneralmatrixEval_voomTest_voomTest) = c("TruthVerif","Confusion1","Confusion2","Confusion3")


    TruthGeneralmatrixEval_voomTest_voomTest[,1] <- GeneralmatrixVerif_voomTest_voomTest[,1]

    for (col in 1:ncol(GeneralmatrixEval_voomTest_voomTest)){
      for (W in 1:nrow(TruthGeneralmatrixEval_voomTest_voomTest)){
        if (TruthGeneralmatrixEval_voomTest_voomTest[W,1] == "Gray" && as.logical(GeneralmatrixEval_voomTest_voomTest[W,col]) == TRUE){
          TruthGeneralmatrixEval_voomTest_voomTest[W,col+1]  = "GrayTrue"
        }else if (TruthGeneralmatrixEval_voomTest_voomTest[W,1] == "Gray" && as.logical(GeneralmatrixEval_voomTest_voomTest[W,col]) == FALSE){
          TruthGeneralmatrixEval_voomTest_voomTest[W,col+1]  = "GrayFalse"
        }else if ( as.logical(TruthGeneralmatrixEval_voomTest_voomTest[W,1]) == TRUE && as.logical(GeneralmatrixEval_voomTest_voomTest[W,col]) == TRUE){
          TruthGeneralmatrixEval_voomTest_voomTest[W,col+1] = "TP"
        } else if (as.logical(TruthGeneralmatrixEval_voomTest_voomTest[W,1]) == FALSE && as.logical(GeneralmatrixEval_voomTest_voomTest[W,col]) == FALSE){
          TruthGeneralmatrixEval_voomTest_voomTest[W,col+1]  = "TN"
        } else if (as.logical(TruthGeneralmatrixEval_voomTest_voomTest[W,1]) == TRUE && as.logical(GeneralmatrixEval_voomTest_voomTest[W,col]) == FALSE){
          TruthGeneralmatrixEval_voomTest_voomTest[W,col+1]  = "FN"
        } else if (as.logical(TruthGeneralmatrixEval_voomTest_voomTest[W,1]) == FALSE && as.logical(GeneralmatrixEval_voomTest_voomTest[W,col]) == TRUE){
          TruthGeneralmatrixEval_voomTest_voomTest[W,col+1]  = "FP"
        }
      }
    }


    voomTest_voomTest.list[[nu]] <- TruthGeneralmatrixEval_voomTest_voomTest
  },
  error = function(e) {
    # if an error occurs, print a message and skip to the next iteration
    message("Error: ", e$message)
...
result.voomTest_voomTest.list <- do.call(rbind, voomTest_voomTest.list)

save(result.voomTest_voomTest.list, file= "./FinalResults/voomTest_voomTest.listTest6.RData")
...
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  ...
  # divide the data into 5 parts and each part is evaluated
  GeneralmatrixEval_aldexTest_aldexTest = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 3)
  GeneralmatrixVerif_aldexTest_aldexTest = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 1)

  # simListH1gs_verif = normaldexTest(physeq = simListH1gs_verif, method = "TMM")
  DAListH1GSverif = aldexTest(simListH1gs_verif)
  matrixVerif = matrix(nrow = nrow(simListH1gs_verif@otu_table), ncol = 1)
  rownames(matrixVerif) = rownames(simListH1gs_verif@otu_table)
  colnames(matrixVerif) = "aldexTest_aldexTest"
  DAListH1GSverif[,2] <- as.logical(DAListH1GSverif[,2] < 0.05)
  for ( x in 1:nrow(DAListH1GSverif)){
    matrixVerif[ rownames(DAListH1GSverif)[x] ,] = as.logical(DAListH1GSverif[,2][x])
  }
  matrixVerif = data.frame(matrixVerif)
  GeneralmatrixVerif_aldexTest_aldexTest[,1] = matrixVerif[,1] # 1 in number of test
  rownames(GeneralmatrixVerif_aldexTest_aldexTest) = rownames(matrixVerif)
  GeneralmatrixVerif_aldexTest_aldexTest = data.frame(GeneralmatrixVerif_aldexTest_aldexTest)
  ...
  for (iter in 1:3){
    ...
    simListH1gs_eval@sam_data = simListH1gs_eval@sam_data[ c(rownames(gr1Shuff),rownames(gr2Shuff)) ,]
    simListH1gs_eval@otu_table = simListH1gs_eval@otu_table[, rownames(simListH1gs_eval@sam_data)]
    # simListH1gs_eval = normaldexTest(physeq = simListH1gs_eval, method = "TMM")
    DAListH1GSeval =  tryCatch(
      aldexTest(simListH1gs_eval),
      error = function(e) e = matrix(nrow =  nrow(simListH1gs_eval@otu_table), ncol = 2)
    )
    # DAListH1GSeval =  aldexTest(simListH1gs_eval)
    # DAListH1GSverif = aldexTest(simListH1gs_verif)
    matrixEval = matrix(nrow = nrow(simListH1gs_eval@otu_table), ncol = 1  )
    rownames(matrixEval) = rownames(simListH1gs_eval@otu_table)
    colnames(matrixEval) = "aldexTest_aldexTest"
    #for (d in 1:length(names(DAListH1GSeval))){
    DAListH1GSeval[,2] <- as.logical(DAListH1GSeval[,2] < 0.05)
    for ( x in 1:nrow(DAListH1GSeval)){
      matrixEval[ rownames(DAListH1GSeval)[x] ,] = as.logical(DAListH1GSeval[,2][x])
    ...
    GeneralmatrixEval_aldexTest_aldexTest[,iter] = matrixEval[,1] # 1 in number of test
    # GeneralmatrixVerif_aldexTest_aldexTest[,iter] = matrixVerif[,1] # 1 in number of test

    rownames(GeneralmatrixEval_aldexTest_aldexTest) = rownames(matrixEval)
    # rownames(GeneralmatrixVerif_aldexTest_aldexTest) = rownames(matrixVerif)
    GeneralmatrixEval_aldexTest_aldexTest = data.frame(GeneralmatrixEval_aldexTest_aldexTest)
    # GeneralmatrixVerif_aldexTest_aldexTest = data.frame(GeneralmatrixVerif_aldexTest_aldexTest)
  }
  GeneralmatrixVerif_aldexTest_aldexTest[is.na(GeneralmatrixVerif_aldexTest_aldexTest)] = FALSE # replace NA with FALSE
  GeneralmatrixEval_aldexTest_aldexTest[is.na(GeneralmatrixEval_aldexTest_aldexTest)] = FALSE # replace NA with FALSE

  TruthGeneralmatrixEval_aldexTest_aldexTest = matrix(nrow = nrow(GeneralmatrixEval_aldexTest_aldexTest),ncol = 4)

  rownames(TruthGeneralmatrixEval_aldexTest_aldexTest) = rownames(GeneralmatrixEval_aldexTest_aldexTest)
  # colnames(TruthGeneralmatrixEval_aldexTest_aldexTest) = c("TruthVerif","Confusion1","Confusion2","Confusion3","Confusion4","Confusion5","GeneralConfusion")
  colnames(TruthGeneralmatrixEval_aldexTest_aldexTest) = c("TruthVerif","Confusion1","Confusion2","Confusion3")


  TruthGeneralmatrixEval_aldexTest_aldexTest[,1] <- GeneralmatrixVerif_aldexTest_aldexTest[,1]

  for (col in 1:ncol(GeneralmatrixEval_aldexTest_aldexTest)){
    for (W in 1:nrow(TruthGeneralmatrixEval_aldexTest_aldexTest)){
      if (TruthGeneralmatrixEval_aldexTest_aldexTest[W,1] == "Gray" && as.logical(GeneralmatrixEval_aldexTest_aldexTest[W,col]) == TRUE){
        TruthGeneralmatrixEval_aldexTest_aldexTest[W,col+1]  = "GrayTrue"
      }else if (TruthGeneralmatrixEval_aldexTest_aldexTest[W,1] == "Gray" && as.logical(GeneralmatrixEval_aldexTest_aldexTest[W,col]) == FALSE){
        TruthGeneralmatrixEval_aldexTest_aldexTest[W,col+1]  = "GrayFalse"
      }else if ( as.logical(TruthGeneralmatrixEval_aldexTest_aldexTest[W,1]) == TRUE && as.logical(GeneralmatrixEval_aldexTest_aldexTest[W,col]) == TRUE){
        TruthGeneralmatrixEval_aldexTest_aldexTest[W,col+1] = "TP"
      } else if (as.logical(TruthGeneralmatrixEval_aldexTest_aldexTest[W,1]) == FALSE && as.logical(GeneralmatrixEval_aldexTest_aldexTest[W,col]) == FALSE){
        TruthGeneralmatrixEval_aldexTest_aldexTest[W,col+1]  = "TN"
      } else if (as.logical(TruthGeneralmatrixEval_aldexTest_aldexTest[W,1]) == TRUE && as.logical(GeneralmatrixEval_aldexTest_aldexTest[W,col]) == FALSE){
        TruthGeneralmatrixEval_aldexTest_aldexTest[W,col+1]  = "FN"
      } else if (as.logical(TruthGeneralmatrixEval_aldexTest_aldexTest[W,1]) == FALSE && as.logical(GeneralmatrixEval_aldexTest_aldexTest[W,col]) == TRUE){
        TruthGeneralmatrixEval_aldexTest_aldexTest[W,col+1]  = "FP"
      }
    }
  }


  aldexTest_aldexTest.list[[nu]] <- TruthGeneralmatrixEval_aldexTest_aldexTest
}
result.aldexTest_aldexTest.list <- do.call(rbind, aldexTest_aldexTest.list)

save(result.aldexTest_aldexTest.list, file= "./FinalResults/aldexTest_aldexTest.listTest6.RData")
######################&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&%%%%%%%%%%%%%%%%%%%%%%%%%%5
############################################&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&%%%%%%%%%%%%%%%%%%%%%%%%%%5
######################&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&%%%%%%%%%%%%%%%%%%%%%%%%%%5
######################&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&%%%%%%%%%%%%%%%%%%%%%%%%%%5
...
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  ...
  # divide the data into 5 parts and each part is evaluated
  GeneralmatrixEval_Ancom60_Ancom60 = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 3)
  GeneralmatrixVerif_Ancom60_Ancom60 = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 1)

  # simListH1gs_verif = normAncom60(physeq = simListH1gs_verif, method = "TMM")
  DAListH1GSverif = Ancom60(simListH1gs_verif)
  matrixVerif = matrix(nrow = nrow(simListH1gs_verif@otu_table), ncol = 1)
  rownames(matrixVerif) = rownames(simListH1gs_verif@otu_table)
  colnames(matrixVerif) = "Ancom60_Ancom60"
  DAListH1GSverif[,2] <- as.logical(DAListH1GSverif[,2] < 0.05)
  for ( x in 1:nrow(DAListH1GSverif)){
    matrixVerif[ rownames(DAListH1GSverif)[x] ,] = as.logical(DAListH1GSverif[,2][x])
  }
  matrixVerif = data.frame(matrixVerif)
  GeneralmatrixVerif_Ancom60_Ancom60[,1] = matrixVerif[,1] # 1 in number of test
  rownames(GeneralmatrixVerif_Ancom60_Ancom60) = rownames(matrixVerif)
  GeneralmatrixVerif_Ancom60_Ancom60 = data.frame(GeneralmatrixVerif_Ancom60_Ancom60)
  ...
  for (iter in 1:3){
    ...
    simListH1gs_eval@sam_data = simListH1gs_eval@sam_data[ c(rownames(gr1Shuff),rownames(gr2Shuff)) ,]
    simListH1gs_eval@otu_table = simListH1gs_eval@otu_table[, rownames(simListH1gs_eval@sam_data)]
    # simListH1gs_eval = normAncom60(physeq = simListH1gs_eval, method = "TMM")
    DAListH1GSeval =  tryCatch(
      Ancom60(simListH1gs_eval),
      error = function(e) e = matrix(nrow =  nrow(simListH1gs_eval@otu_table), ncol = 2)
    )
    # DAListH1GSeval =  Ancom60(simListH1gs_eval)
    # DAListH1GSverif = Ancom60(simListH1gs_verif)
    matrixEval = matrix(nrow = nrow(simListH1gs_eval@otu_table), ncol = 1  )
    rownames(matrixEval) = rownames(simListH1gs_eval@otu_table)
    colnames(matrixEval) = "Ancom60_Ancom60"
    #for (d in 1:length(names(DAListH1GSeval))){
    DAListH1GSeval[,2] <- as.logical(DAListH1GSeval[,2] < 0.05)
    for ( x in 1:nrow(DAListH1GSeval)){
      matrixEval[ rownames(DAListH1GSeval)[x] ,] = as.logical(DAListH1GSeval[,2][x])
    ...
    GeneralmatrixEval_Ancom60_Ancom60[,iter] = matrixEval[,1] # 1 in number of test
    # GeneralmatrixVerif_Ancom60_Ancom60[,iter] = matrixVerif[,1] # 1 in number of test

    rownames(GeneralmatrixEval_Ancom60_Ancom60) = rownames(matrixEval)
    # rownames(GeneralmatrixVerif_Ancom60_Ancom60) = rownames(matrixVerif)
    GeneralmatrixEval_Ancom60_Ancom60 = data.frame(GeneralmatrixEval_Ancom60_Ancom60)
    # GeneralmatrixVerif_Ancom60_Ancom60 = data.frame(GeneralmatrixVerif_Ancom60_Ancom60)
  }
  GeneralmatrixVerif_Ancom60_Ancom60[is.na(GeneralmatrixVerif_Ancom60_Ancom60)] = FALSE # replace NA with FALSE
  GeneralmatrixEval_Ancom60_Ancom60[is.na(GeneralmatrixEval_Ancom60_Ancom60)] = FALSE # replace NA with FALSE

  TruthGeneralmatrixEval_Ancom60_Ancom60 = matrix(nrow = nrow(GeneralmatrixEval_Ancom60_Ancom60),ncol = 4)

  rownames(TruthGeneralmatrixEval_Ancom60_Ancom60) = rownames(GeneralmatrixEval_Ancom60_Ancom60)
  # colnames(TruthGeneralmatrixEval_Ancom60_Ancom60) = c("TruthVerif","Confusion1","Confusion2","Confusion3","Confusion4","Confusion5","GeneralConfusion")
  colnames(TruthGeneralmatrixEval_Ancom60_Ancom60) = c("TruthVerif","Confusion1","Confusion2","Confusion3")


  TruthGeneralmatrixEval_Ancom60_Ancom60[,1] <- GeneralmatrixVerif_Ancom60_Ancom60[,1]

  for (col in 1:ncol(GeneralmatrixEval_Ancom60_Ancom60)){
    for (W in 1:nrow(TruthGeneralmatrixEval_Ancom60_Ancom60)){
      if (TruthGeneralmatrixEval_Ancom60_Ancom60[W,1] == "Gray" && as.logical(GeneralmatrixEval_Ancom60_Ancom60[W,col]) == TRUE){
        TruthGeneralmatrixEval_Ancom60_Ancom60[W,col+1]  = "GrayTrue"
      }else if (TruthGeneralmatrixEval_Ancom60_Ancom60[W,1] == "Gray" && as.logical(GeneralmatrixEval_Ancom60_Ancom60[W,col]) == FALSE){
        TruthGeneralmatrixEval_Ancom60_Ancom60[W,col+1]  = "GrayFalse"
      }else if ( as.logical(TruthGeneralmatrixEval_Ancom60_Ancom60[W,1]) == TRUE && as.logical(GeneralmatrixEval_Ancom60_Ancom60[W,col]) == TRUE){
        TruthGeneralmatrixEval_Ancom60_Ancom60[W,col+1] = "TP"
      } else if (as.logical(TruthGeneralmatrixEval_Ancom60_Ancom60[W,1]) == FALSE && as.logical(GeneralmatrixEval_Ancom60_Ancom60[W,col]) == FALSE){
        TruthGeneralmatrixEval_Ancom60_Ancom60[W,col+1]  = "TN"
      } else if (as.logical(TruthGeneralmatrixEval_Ancom60_Ancom60[W,1]) == TRUE && as.logical(GeneralmatrixEval_Ancom60_Ancom60[W,col]) == FALSE){
        TruthGeneralmatrixEval_Ancom60_Ancom60[W,col+1]  = "FN"
      } else if (as.logical(TruthGeneralmatrixEval_Ancom60_Ancom60[W,1]) == FALSE && as.logical(GeneralmatrixEval_Ancom60_Ancom60[W,col]) == TRUE){
        TruthGeneralmatrixEval_Ancom60_Ancom60[W,col+1]  = "FP"
      }
    }
  }


  Ancom60_Ancom60.list[[nu]] <- TruthGeneralmatrixEval_Ancom60_Ancom60
}
result.Ancom60_Ancom60.list <- do.call(rbind, Ancom60_Ancom60.list)

save(result.Ancom60_Ancom60.list, file= "./FinalResults/Ancom60_Ancom60.listTest6.RData")
...
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  ...
  simListH1gs_verif = simListH1gs[[nu]]
  # divide the data into 5 parts and each part is evaluated
  GeneralmatrixEval_GEECLR_GEECLR = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 3)
  GeneralmatrixVerif_GEECLR_GEECLR = matrix(nrow = nrow(simListH1gs[[nu]]@otu_table), ncol = 1)

  DAListH1GSverif = GEECLR(simListH1gs_verif)
  matrixVerif = matrix(nrow = nrow(simListH1gs_verif@otu_table), ncol = 1)
  rownames(matrixVerif) = rownames(simListH1gs_verif@otu_table)
  colnames(matrixVerif) = "GEECLR_GEECLR"
  DAListH1GSverif[,2] <- as.logical(DAListH1GSverif[,2] < 0.05)
  for ( x in 1:nrow(DAListH1GSverif)){
    matrixVerif[ rownames(DAListH1GSverif)[x] ,] = as.logical(DAListH1GSverif[,2][x])
  }
  matrixVerif = data.frame(matrixVerif)
  GeneralmatrixVerif_GEECLR_GEECLR[,1] = matrixVerif[,1] # 1 in number of test
  rownames(GeneralmatrixVerif_GEECLR_GEECLR) = rownames(matrixVerif)
  GeneralmatrixVerif_GEECLR_GEECLR = data.frame(GeneralmatrixVerif_GEECLR_GEECLR)
  ...
  for (iter in 1:3){
    ...
    gr2 = simListH1gs[[nu]]@sam_data[simListH1gs[[nu]]@sam_data$group == "grp2" ,]
    # cross validation
    gr1Shuff = gr1[ round( (1:(nrow(gr1)/3) + ( (iter-1) * (nrow(gr1)/3)) ) )]  #gr1[sample(nrow(gr1),5)]
    gr2Shuff =  gr2[ round( (1:(nrow(gr2)/3) + ( (iter-1) * (nrow(gr2)/3)) ) )] #gr2[sample(nrow(gr2),5)]
    simListH1gs_eval@sam_data = simListH1gs_eval@sam_data[ c(rownames(gr1Shuff),rownames(gr2Shuff)) ,]
    simListH1gs_eval@otu_table = simListH1gs_eval@otu_table[, rownames(simListH1gs_eval@sam_data)]
    # simListH1gs_verif@sam_data = simListH1gs_verif@sam_data[ !row.names(simListH1gs_verif@sam_data) %in% c(rownames(gr1Shuff),rownames(gr2Shuff)),]
    # simListH1gs_verif@otu_table = simListH1gs_verif@otu_table[, rownames(simListH1gs_verif@sam_data)]
    DAListH1GSeval =  tryCatch(
      GEECLR(simListH1gs_eval),
      error = function(e) e = matrix(nrow =  nrow(simListH1gs_eval@otu_table), ncol = 2)
    )
    # DAListH1GSeval =  GEECLR(simListH1gs_eval)
    # DAListH1GSverif = GEECLR(simListH1gs_verif)
    matrixEval = matrix(nrow = nrow(simListH1gs_eval@otu_table), ncol = 1  )
    rownames(matrixEval) = rownames(simListH1gs_eval@otu_table)
    colnames(matrixEval) = "GEECLR_GEECLR"
    #for (d in 1:length(names(DAListH1GSeval))){
    DAListH1GSeval[,2] <- as.logical(DAListH1GSeval[,2] < 0.05)
    for ( x in 1:nrow(DAListH1GSeval)){
      matrixEval[ rownames(DAListH1GSeval)[x] ,] = as.logical(DAListH1GSeval[,2][x])
    ...
    GeneralmatrixEval_GEECLR_GEECLR[,iter] = matrixEval[,1] # 1 in number of test
    # GeneralmatrixVerif_GEECLR_GEECLR[,iter] = matrixVerif[,1] # 1 in number of test

    rownames(GeneralmatrixEval_GEECLR_GEECLR) = rownames(matrixEval)
    # rownames(GeneralmatrixVerif_GEECLR_GEECLR) = rownames(matrixVerif)
    GeneralmatrixEval_GEECLR_GEECLR = data.frame(GeneralmatrixEval_GEECLR_GEECLR)
    # GeneralmatrixVerif_GEECLR_GEECLR = data.frame(GeneralmatrixVerif_GEECLR_GEECLR)
  }
  GeneralmatrixVerif_GEECLR_GEECLR[is.na(GeneralmatrixVerif_GEECLR_GEECLR)] = FALSE # replace NA with FALSE
  GeneralmatrixEval_GEECLR_GEECLR[is.na(GeneralmatrixEval_GEECLR_GEECLR)] = FALSE # replace NA with FALSE

  TruthGeneralmatrixEval_GEECLR_GEECLR = matrix(nrow = nrow(GeneralmatrixEval_GEECLR_GEECLR),ncol = 4)

  rownames(TruthGeneralmatrixEval_GEECLR_GEECLR) = rownames(GeneralmatrixEval_GEECLR_GEECLR)
  # colnames(TruthGeneralmatrixEval_GEECLR_GEECLR) = c("TruthVerif","Confusion1","Confusion2","Confusion3","Confusion4","Confusion5","GeneralConfusion")
  colnames(TruthGeneralmatrixEval_GEECLR_GEECLR) = c("TruthVerif","Confusion1","Confusion2","Confusion3")

  # for (truth in 1:nrow(GeneralmatrixVerif_GEECLR_GEECLR)){
  #   if (sum(GeneralmatrixVerif_GEECLR_GEECLR[truth,], na.rm=T) > 1){
  #     TruthGeneralmatrixEval_GEECLR_GEECLR[truth,1] = c("TRUE")
  #   } else if (sum(GeneralmatrixVerif_GEECLR_GEECLR[truth,], na.rm=T) < 2){
  #     TruthGeneralmatrixEval_GEECLR_GEECLR[truth,1] = c("FALSE")
  #   } else {TruthGeneralmatrixEval_GEECLR_GEECLR[truth,1] = c("Gray")}
  # }
  TruthGeneralmatrixEval_GEECLR_GEECLR[,1] <- GeneralmatrixVerif_GEECLR_GEECLR[,1]

  for (col in 1:ncol(GeneralmatrixEval_GEECLR_GEECLR)){
    for (W in 1:nrow(TruthGeneralmatrixEval_GEECLR_GEECLR)){
      if (TruthGeneralmatrixEval_GEECLR_GEECLR[W,1] == "Gray" && as.logical(GeneralmatrixEval_GEECLR_GEECLR[W,col]) == TRUE){
        TruthGeneralmatrixEval_GEECLR_GEECLR[W,col+1]  = "GrayTrue"
      }else if (TruthGeneralmatrixEval_GEECLR_GEECLR[W,1] == "Gray" && as.logical(GeneralmatrixEval_GEECLR_GEECLR[W,col]) == FALSE){
        TruthGeneralmatrixEval_GEECLR_GEECLR[W,col+1]  = "GrayFalse"
      }else if ( as.logical(TruthGeneralmatrixEval_GEECLR_GEECLR[W,1]) == TRUE && as.logical(GeneralmatrixEval_GEECLR_GEECLR[W,col]) == TRUE){
        TruthGeneralmatrixEval_GEECLR_GEECLR[W,col+1] = "TP"
      } else if (as.logical(TruthGeneralmatrixEval_GEECLR_GEECLR[W,1]) == FALSE && as.logical(GeneralmatrixEval_GEECLR_GEECLR[W,col]) == FALSE){
        TruthGeneralmatrixEval_GEECLR_GEECLR[W,col+1]  = "TN"
      } else if (as.logical(TruthGeneralmatrixEval_GEECLR_GEECLR[W,1]) == TRUE && as.logical(GeneralmatrixEval_GEECLR_GEECLR[W,col]) == FALSE){
        TruthGeneralmatrixEval_GEECLR_GEECLR[W,col+1]  = "FN"
      } else if (as.logical(TruthGeneralmatrixEval_GEECLR_GEECLR[W,1]) == FALSE && as.logical(GeneralmatrixEval_GEECLR_GEECLR[W,col]) == TRUE){
        TruthGeneralmatrixEval_GEECLR_GEECLR[W,col+1]  = "FP"
      }
    }
  }
  # for (W in 1:nrow(TruthGeneralmatrixEval_GEECLR_GEECLR)){
  #   if (any( TruthGeneralmatrixEval_GEECLR_GEECLR[W,2:5] == "FP")){
  #     TruthGeneralmatrixEval_GEECLR_GEECLR[W,7] = "FP"
  #   } else if (any( TruthGeneralmatrixEval_GEECLR_GEECLR[W,2:5] == "TP")){
  #     TruthGeneralmatrixEval_GEECLR_GEECLR[W,7] = "TP"
  #   } else {
  #     TruthGeneralmatrixEval_GEECLR_GEECLR[W,7] = tail(names(sort(table(TruthGeneralmatrixEval_GEECLR_GEECLR[W,2:5]))), 1)
  #   }
  # }

  # for (i in 1:nrow(TruthGeneralmatrixEval_GEECLR_GEECLR)){
  #   # add -TP to TAXA names as ground truth
  #   if (TruthGeneralmatrixEval_GEECLR_GEECLR[i,1] == TRUE){
  #     rownames(TruthGeneralmatrixEval_GEECLR_GEECLR)[i] = paste0(rownames(TruthGeneralmatrixEval_GEECLR_GEECLR)[i],"-TP")
  #   }
  #   # detect the significant in small data
  #   if (TruthGeneralmatrixEval_GEECLR_GEECLR[i,7] == "TP" ||TruthGeneralmatrixEval_GEECLR_GEECLR[i,7] == "FP"){
  #     TruthGeneralmatrixEval_GEECLR_GEECLR[i,7] = 0
  #   } else {TruthGeneralmatrixEval_GEECLR_GEECLR[i,7] = 1}
  # }
  # GEECLR_GEECLR =as.data.frame(cbind(as.numeric(TruthGeneralmatrixEval_GEECLR_GEECLR[,7]), as.numeric(TruthGeneralmatrixEval_GEECLR_GEECLR[,7])))
  # colnames(GEECLR_GEECLR)[1] = "rawP"
  # colnames(GEECLR_GEECLR)[2] = "adjP"
  # rownames(GEECLR_GEECLR) = rownames(TruthGeneralmatrixEval_GEECLR_GEECLR)

  # GEECLR_GEECLR.list[[nu]] <- GEECLR_GEECLR

  GEECLR_GEECLR.list[[nu]] <- TruthGeneralmatrixEval_GEECLR_GEECLR

  cat("done <<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>",nu)
}
result.GEECLR_GEECLR.list <- do.call(rbind, GEECLR_GEECLR.list)

save(GEECLR_GEECLR.list, file= "./FinalResults/GEECLR_GEECLR.listTest6Edit100taxa.RData")

TotalResultsList.GEECLR = table(result.GEECLR_GEECLR.list[,2:4])
# confusionMatrix.GEECLR = matrix(ncol = 4,nrow = 1)
# confusionMatrix.GEECLR[1,1] = 0
# confusionMatrix.GEECLR[1,2] = 0
# confusionMatrix.GEECLR[1,3] = 0
# confusionMatrix.GEECLR[1,4] = 0
#for (i in 1:length(TotalResultsList)){
#j = names(TotalResultsList[[1]])[1] # names(TotalResultsList[[1]])
#for (j in 1:length(names(TotalResultsList))){
#TotalResultsList[[i]][[j]]
# Test_Matrix = matrix(nrow = nrow(TotalResultsList),ncol = 3)
# Test_Matrix[,1] =  as.integer(grepl("-TP", rownames(TotalResultsList))) # if -TP back 1 as significant
# Test_Matrix[,2] =  1 - TotalResultsList[,2]
# for (w in 1:nrow(TotalResultsList)) {
...
Results_GEECLR_GEECLR = matrix(ncol = 4,nrow = 1)
if (is.na(TotalResultsList.GEECLR["TP"] )){
  TotalResultsList.GEECLR["TP"] = 0
} else if(is.na(TotalResultsList.GEECLR["TN"] )){
  TotalResultsList.GEECLR["TN"] = 0
}else if(is.na(TotalResultsList.GEECLR["FP"] )){
  TotalResultsList.GEECLR["FP"] = 0
}else if(is.na(TotalResultsList.GEECLR["FN"] )){
  TotalResultsList.GEECLR["FN"] = 0
}
Results_GEECLR_GEECLR[1,1] = TotalResultsList.GEECLR["TP"] / (TotalResultsList.GEECLR["TP"] + TotalResultsList.GEECLR["FN"])
Results_GEECLR_GEECLR[1,2] = TotalResultsList.GEECLR["TN"] / (TotalResultsList.GEECLR["TN"] + TotalResultsList.GEECLR["FP"])
Results_GEECLR_GEECLR[1,3] = TotalResultsList.GEECLR["FP"] / (TotalResultsList.GEECLR["FP"] + TotalResultsList.GEECLR["TP"])
Results_GEECLR_GEECLR[1,4] = TotalResultsList.GEECLR["FP"] / (TotalResultsList.GEECLR["FP"] + TotalResultsList.GEECLR["TN"])
...
colnames(Results_GEECLR_GEECLR)[4] = "FPR"
Results_GEECLR_GEECLR[is.na(Results_GEECLR_GEECLR)] <- 0
Results_GEECLR_GEECLR
#####################&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&%%%%%%%%%%%%%%%%%%%%%%%%%%5
###############################################  After edition ###########################################################
##########################################################################################################
GEECLR_GEECLR.list <- list()
...
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  #tryCatch({
    ...
    # imListH1gs[[nu]]
    # randomize the rows
    #simListH1gs_eval@sam_data = simListH1gs_eval@sam_data[sample(nrow(simListH1gs_eval@sam_data)) ,]


  for (iter in 1:3){
        simListH1gs_eval = simListH1gs_verif
        # simListH1gs_verif = simListH1gs[[nu]]
        # gr1 = simListH1gs[[nu]]@sam_data[simListH1gs[[nu]]@sam_data$group == "grp1" ,]
        # gr2 = simListH1gs[[nu]]@sam_data[simListH1gs[[nu]]@sam_data$group == "grp2" ,]
        gr1 = simListH1gs_eval@sam_data[simListH1gs_eval@sam_data$group == "grp1" ,]
        gr2 = simListH1gs_eval@sam_data[simListH1gs_eval@sam_data$group == "grp2" ,]
        # cross validation
        gr1Shuff = gr1[ (1:(nrow(gr1)/3) + ( (iter-1) * (nrow(gr1)/3)) ) ]  #gr1[sample(nrow(gr1),5)]
        gr2Shuff =  gr2[ (1:(nrow(gr2)/3) + ( (iter-1) * (nrow(gr2)/3)) ) ] #gr2[sample(nrow(gr2),5)]
        simListH1gs_eval@sam_data = simListH1gs_eval@sam_data[ c(rownames(gr1Shuff),rownames(gr2Shuff)) ,]
...
#save(GEECLR_GEECLR.list, file= "./FinalResults/GEECLR_GEECLR.listTest6Edit100taxa.RData")
save(GEECLR_GEECLR.list, file= "./FinalResults/GEECLR_GEECLR.listTest6Edit2.100taxa.RData")
##########################################################################################################
##########################################################################################################
##########################################################################################################
##########################################################################################################
  ...
  tryCatch({
    ...
    for (iter in 1:3){
      ...
      simListH1gs_eval@otu_table = simListH1gs_eval@otu_table[, rownames(simListH1gs_eval@sam_data)]
        ...
        # add the result of small data into iteraty list
        twoWayANOVA_twoWayANOVA.list[[(length(twoWayANOVA_twoWayANOVA.list) + 1)]] <- DAListH1GSeval
      }
  },
  error = function(e) {
    # if an error occurs, print a message and skip to the next iteration
...
save(twoWayANOVA_twoWayANOVA.list, file= "./FinalResults/twoWayANOVA_twoWayANOVA.listTest6Edit100taxa.RData")
###########################################################################################################
###########################################################################################################
###########################################################################################################
###########################################################################################################
...
save(edgeR_edgeR.list, file= "./FinalResults/edgeR_edgeR.listTest6Edit100taxa.RData")
###########################################################################################################
###########################################################################################################
###########################################################################################################
###########################################################################################################
...
save(deseq2_deseq2.list, file= "./FinalResults/deseq2_deseq2.listTest6Edit100taxa.RData")
###########################################################################################################
###########################################################################################################
###########################################################################################################
###########################################################################################################
...
save(voomTest_voomTest.list, file= "./FinalResults/voomTest_voomTest.listTest6Edit100taxa.RData")
###########################################################################################################
###########################################################################################################
###########################################################################################################
###########################################################################################################
...
save(aldexTest_aldexTest.list, file= "./FinalResults/aldexTest_aldexTest.listTest6Edit100taxa.RData")
###########################################################################################################
###########################################################################################################
###########################################################################################################
###########################################################################################################
...
nu = args
load( "./FinalResults/Ancom60_Ancom60.listTest6Edit100taxa.RData")
#for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  tryCatch({
    simListH1gs_verif = simListH1gs[[nu]]
  ...
  cat("done <<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>",nu)
#}
save(Ancom60_Ancom60.list, file= "./FinalResults/Ancom60_Ancom60.listTest6Edit100taxa.RData")
#########################################################################################################
#########################################################################################################
#########################################################################################################
#########################################################################################################
...
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  tryCatch({
    ...
    #simListH1gs_verif = normEdgeR(physeq = simListH1gs_verif, method = "TMM")
    DAListH1GSverif = TimeSeries_ancombcTest2(simListH1gs_verif)
    ...
    for (iter in 1:3){
      ...
      #simListH1gs_eval = normEdgeR(physeq = simListH1gs_eval, method = "TMM")
      DAListH1GSeval =  tryCatch(
        TimeSeries_ancombcTest2(simListH1gs_eval),
        error = function(e) e = matrix(nrow =  nrow(simListH1gs_eval@otu_table), ncol = 2)
      )
...
save(ancombc2_ancombc2.list, file= "./FinalResults/ancombc2_ancombc2.listTest6Edit100taxa.RData")
# ###########################################################################################################
###########################################################################################################
###########################################################################################################
###########################################################################################################
...
for (nu in 1:length(simListH1gs)){  # 1:length(simListH1gs)
  #tryCatch({
    ...
    simListH1gs_verif = normEdgeR(physeq = simListH1gs_verif, method = "TMM")
    DAListH1GSverif = TimeSeries_metagenomeSeqZIG(simListH1gs_verif, normFacts = "TMM")
    ...
    for (iter in 1:3){
      ...
      simListH1gs_eval = normEdgeR(physeq = simListH1gs_eval, method = "TMM")
      DAListH1GSeval =  tryCatch(
        TimeSeries_metagenomeSeqZIG(simListH1gs_eval, normFacts = "TMM"),
        error = function(e) e = matrix(nrow =  nrow(simListH1gs_eval@otu_table), ncol = 2)
      )
...
# save(metagenomeSeq_metagenomeSeq.list, file= "./FinalResults/metagenomeSeq_metagenomeSeq.listTest6Edit100taxa.RData")
save(metagenomeSeq_metagenomeSeq.list, file= "./FinalResults/metagenomeSeq_metagenomeSeq.listTest6Edit2.100taxa.RData")
##########################################################################################################
##########################################################################################################
load("./FinalResults/ancombc2_ancombc2.listTest6Edit100taxa.RData")
load("./FinalResults/Ancom60_Ancom60.listTest6Edit100taxa.RData")
load("./FinalResults/aldexTest_aldexTest.listTest6Edit100taxa.RData")
load("./FinalResults/voomTest_voomTest.listTest6Edit100taxa.RData")
load("./FinalResults/deseq2_deseq2.listTest6Edit100taxa.RData")
load("./FinalResults/edgeR_edgeR.listTest6Edit100taxa.RData")
load("./FinalResults/twoWayANOVA_twoWayANOVA.listTest6Edit100taxa.RData")
load("./FinalResults/GEECLR_GEECLR.listTest6Edit2.100taxa.RData")
load( "./FinalResults/metagenomeSeq_metagenomeSeq.listTest6Edit2.100taxa.RData")
load("./FinalResults/GEECLR3_GEECLR3.listTest6Edit2.100taxa.RData")
load("./FinalResults/GEECLR2_GEECLR2.listTest6Edit2.100taxa.RData")

#load("./FinalResults/GEECLR_GEECLR.listTest6Edit100taxa.RData")

DAlistAncomNoCompOutliers <- list()

for (i in 1:length(metagenomeSeq_metagenomeSeq.list)){
  ListTests <- list(GEE_CLRCTF = GEECLR3_GEECLR3.list[[i]], Ancom60_Ancom60 = Ancom60_Ancom60.list[[i]], mgsZig_TMM = metagenomeSeq_metagenomeSeq.list[[i]],
                    ...
                    twoWayANOVA_twoWayANOVA = twoWayANOVA_twoWayANOVA.list[[i]], ancombc2_ancombc2 = ancombc2_ancombc2.list[[i]])
  DAlistAncomNoCompOutliers[[i]] <- ListTests
}
save(DAlistAncomNoCompOutliers, file = "./FinalCode/ResultTest6_DAlistAncomNoCompOutliers.RData")

load("./FinalCode/ResultTest6_DAlistAncomNoCompOutliers.RData")
###################%%%%%%%%%%%%%%%%%%%%%%%%%%%
######################################%%%%%%%%%%%%%%%%%%%%%%%%%%%
#########################################################%%%%%%%%%%%%%%%%%%%%%%%%%%%
############################################################################%%%%%%%%%%%%%%%%%%%%%%%%%%%
# apply statistics test on the truth result
DAlistAncomNoCompOutliers = DAlistAncomNoCompOutliers
...
write.csv(WilcoxPostHocSenstivityResult$p.value, file = "DepRealSen.csv")
########$$$$$$$$$$$
## For Specificity 
...
write.csv(WilcoxPostHocspecificityResult$p.value, file = "DepRealSpe.csv")
###############################################################################################%%%%%%%%%%%%%%%%%%%%%%%%%%%
##################################################################################################################%%%%%%%%%%%%%%%%%%%%%%%%%%%
## For FDR 
...
write.csv(WilcoxPostHocFDRResult$p.value, file = "DepRealFDR.csv")
###############################################################################################%%%%%%%%%%%%%%%%%%%%%%%%%%%
##################################################################################################################%%%%%%%%%%%%%%%%%%%%%%%%%%%
###################%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for (i in 1:length(TotalResultsList)){
  tryCatch({
    j = names(TotalResultsList[[1]])[4] # names(TotalResultsList[[1]])
...
Results_twoWayANOVA_twoWayANOVA = matrix(ncol = 3,nrow = 1)
Results_twoWayANOVA_twoWayANOVA[1,1] = confusionMatrix[1,1] / (confusionMatrix[1,1] + confusionMatrix[1,4])
Results_twoWayANOVA_twoWayANOVA[1,2] = confusionMatrix[1,2] / (confusionMatrix[1,2] + confusionMatrix[1,3])
Results_twoWayANOVA_twoWayANOVA[1,3] = confusionMatrix[1,3] / (confusionMatrix[1,1] + confusionMatrix[1,3])
Results_twoWayANOVA_twoWayANOVA = as.data.frame(Results_twoWayANOVA_twoWayANOVA)
colnames(Results_twoWayANOVA_twoWayANOVA)[1] = "sensitivity"
colnames(Results_twoWayANOVA_twoWayANOVA)[2] = "specificity"
colnames(Results_twoWayANOVA_twoWayANOVA)[3] = "FDR"
Results_twoWayANOVA_twoWayANOVA[is.na(Results_twoWayANOVA_twoWayANOVA)] <- 0
...
sensitivityValue <- c(0, 28.4, 12.6, 30.6, 0, 0, 14.2, 0, 16)
specificityValue <- c(98.3, 92.5, 95.5, 91.7, 99.7, 99.9, 99.3, 100, 99.5)
FDRValue <- c(100, 97.3, 50.2, 61.03, 100, 100, 97.5, 0, 30.08)

# calculate standard error std.error()
sensitivityError <- c(0, 39.4, 14.66, 22.69, 0, 0, 13.054, 0, 30.61)
specificityError <- c(15.613, 7.283, 18.245, 46.06, 16.732, 18.296, 0.854, 0, 0.759)
...
#data_long$sd <- c(0, 17.320216, 8.147008, 20.055227, 2.972942, 2.227015, 11.638928, 0,11.932470,0, 4.1614891, 1.6581905, 5.6331517, 0.2222222, 0, 2.0524038, 0, 0.6334436)
data_long$sd <- c(0, 39.4, 14.66, 22.69, 0, 0, 13.054, 0, 30.61, 15.613, 7.283, 18.245, 46.06, 16.732, 18.296, 0.854, 0, 0.759)
...
# Open a svg file
png("./FinalCode/Test6Result.png", units = "in", width = 12, height = 5, res = 100 )
# Create the double bar plot using ggplot2
...
#   Sensitivity = c(0.1, 28.4, 12.6, 30.6, 0.1, 0.1, 14.2, 0.1, 13.2),
#   Specificity = c(98.3, 92.5, 95.5, 91.7, 99.7, 99.9, 99.3, 100, 99.5),
#   FDR = 100 - c(100, 97.3, 50.2, 61.03, 100, 100, 97.5, 0.1, 30.08)
# )
scores <- data.frame(
  row.names = c( "DESeq2","edgeR", "MetagenomeSeq" ,"limma-voom", "ALDEx2", "ANCOM", " ANCOM-BC2", "GEE-CLR"),
  Sensitivity = c(28.4, 12.6, 30.6, 0.1, 0.1, 14.2, 0.1, 13.2),
  Specificity = c(92.5, 95.5, 91.7, 99.7, 99.9, 99.3, 100, 99.5),
  FDR = 100 - c(97.3, 50.2, 61.03, 100, 100, 97.5, 0.1, 30.08)
)
...
head(df3)
#df3["SD"] <- c(0, 15.613, 39.4, 7.283, 14.66, 18.245, 22.69, 46.06, 0, 16.732, 0, 18.296, 13.054, 0.854, 0, 0, 30.61, 0.759)
##############
# Open a svg file
library(Cairo)
...
Cairo::Cairo(
  30, #length
  30, #width
  file = paste("Spider_Dependent_Real", ".png", sep = ""),
  type = "png", #tiff
...
ggdotchart(
  ...
  sorting = "none", title = "Dependent real evaluated data",
...
png("./FinalCode/Test6TP.png", units = "in", width = 14, height = 10, res = 100 )
...
png("./FinalCode/Test6FP.png", units = "in", width = 14, height = 10, res = 100 )
ggvenn(parameters, show_elements = F, stroke_color = "black", stroke_linetype = "solid",
       show_percentage = F, fill_color = c('#00BFC4','#C77CFF','#F8766D', "#009E73") ,
       text_size = 15, set_name_size = 7, set_name_color = c('#00BFC4','#C77CFF','#F8766D', "#009E73") ,
...
png("./FinalCode/Test6Truth.png", units = "in", width = 14, height = 10, res = 100 )
ggvenn(parameters, show_elements = F, stroke_color = "black", stroke_linetype = "solid",
       show_percentage = F, fill_color = c('#00BFC4','#C77CFF','#F8766D', "#009E73") ,
       text_size = 15, set_name_size = 7, set_name_color = c('#00BFC4','#C77CFF','#F8766D', "#009E73") ,
